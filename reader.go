...
func (p Packet) SetSourcePort(port uint16) {
	p[0], p[1] = byte(port>>8), byte((port<<8)>>8)
}
...
func (p Packet) SetDestinationPort(port uint16) {
	p[2], p[3] = byte(port>>8), byte((port<<8)>>8)
}
...
func (p Packet) SetWindow(port uint16) {
	p[14], p[15] = byte(port>>8), byte((port<<8)>>8)
}
...
func (p Packet) SetChecksum(port uint16) {
	p[16], p[17] = byte(port>>8), byte((port<<8)>>8)
}
...
func (p Packet) SetUrgentPointer(port uint16) {
	p[18], p[19] = byte(port>>8), byte((port<<8)>>8)
}
